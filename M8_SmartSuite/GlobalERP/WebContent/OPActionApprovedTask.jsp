...
function Redirect(indxtype,dept,ename,frdate,todate)
{
    ...
    window.location="OPTaskMailDownLodedDataReport.jsp?Index="+indxtype+"&companyName1="+dept+"&EmpName="+ename+"&from_date1="+frdate+"&to_date1="+todate;
}
...
for(int i=1;i<arr.length;i++)
{
 ...
 /*   String sql_total="select * from dotproject.tasks where task_id='"+arr[i]+"'";
	...
	String MailDateTime="";
	String user="select * from db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"taskmaildataCopy where TaskID='"+task_id+"' ";
	...
	if(rs1.next())
			...
			MailDateTime=rs1.getString("MailDateTime");	
				...
				String userdetails="select a.user_id,b.EmpName,b.Dept from dotproject.user_tasks a ,db_GlobalERP.UserMaster b where a.user_id=b.dotprojectid and task_id='"+task_id+"'";
...
try{
	...
	if(action.equals("approve"))
	{
		...
		String updatetask="update db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"taskmaildataCopy set TaskStatus='Approved' where TaskID='"+task_id+"'";
	  ...
	  //  out.println("location='TaskLog.jsp?projectid="+Project_id+"&taskid="+taskid+"&totalTask="+totalTask+"&frmDate="+frmDate+"&toTime="+toTime+"&user_id="+Userid+"&user_name="+Username+"'");
	    ...
	    else
		{
	    	...
	    	int count=0;
			String updatetask="update db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"taskmaildataCopy set TaskStatus='Rejected',TaskRejectionComment='"+rejcomment2[i]+"' where TaskID='"+task_id+"'";
			System.out.println("UPDATE QUERRY====>"+updatetask);
