...
try {
	con = connection.getConnect("dotproject");    
    st = con.createStatement();
    stmt=con.createStatement();
    stmt1=con.createStatement();
    stmain1=con.createStatement();
    stmain2=con.createStatement();
        ...
        {	
         ...
         <tr> 
						...
						<%     
							if(Sesrole2.equals("AllAdmin") || Sesrole2.equals("HOD"))
						{%>
						<td class="text-right"> <a data-bd-toggle="modal" style="color: #0d6efd;cursor:pointer;" data-bs-target="#addMyModall" onclick="return demoo('<%=datenew1%>','<%=datenew2%>','<%=HOD%>','<%=department%>','<%=rs.getString("User")%>','<%=Status%>','<%=Role%>','<%=rs.getString("TaskName")%>','<%=rs.getString("UserID")%>','<%=rs.getString("TaskId")%>')"><%=rs.getString("TaskId")%></a></td>
				<%	}else
				{%>
 ...
 %>
        ...
        {        	
			...
			<tr>
						...
						<td class="text-right"><%=UserID%></td>
 						<td class="text-center">
 							<a data-bs-toggle="modal" data-bs-target="#addMyModal" onclick="return demo('<%=datenew1%>','<%=datenew2%>','<%=HOD%>','<%=department%>','<%=rs.getString("User")%>','<%=Status%>','<%=Role%>','<%=rs.getString("TaskName")%>','<%=rs.getString("UserID")%>','<%=rs.getString("TaskId")%>')"><%=rs.getString("TaskId")%></a>
 						<td class="text-left"><%=TaskName%></td>
 						<td class="text-left"><%=task_description%></td>
						<td class="text-right"><%=Duration%></td>
						<td class="text-right"><%=StartDate%></td>
...
<%        				if(Sesrole2.equals("AllAdmin") || Sesrole2.equals("HOD"))
							{%>
					<td class="text-right"> 
						<a data-bs-toggle="modal" data-bs-target="#addMyModall" style="color: #0d6efd;cursor:pointer;" onclick="return demoo('<%=datenew1%>','<%=datenew2%>','<%=HOD%>','<%=department%>','<%=User%>','<%=Status%>','<%=Role%>','<%=TaskName%>','<%=UserID%>','<%=TaskId%>','<%=task_project%>')"><%=TaskId%></a></td>
					<%	}else
					{%>
					<td class="text-right" style="color: #0d6efd;cursor:pointer;"><%=TaskId%></td>
				<%}
 %>
					<td class="text-left"><%=TaskName%></td>
					<td class="text-left"><%=task_description%></td>				
...
//			Stat=rs.getString("Status");
		...
		<tr>
					...
					<td class="text-right"><%=UserID%></td>
					<td style="font-size: 11px;" class="text-center">
					 	<a data-bs-toggle="modal" data-bs-target="#addMyModal" style="color: #0d6efd;cursor:pointer;" onclick="return demo('<%=datenew1%>','<%=datenew2%>','<%=HOD%>','<%=department%>','<%=rs.getString("User")%>','<%=Status%>','<%=Role%>','<%=TaskName%>','<%=UserID%>','<%=TaskId%>')"><%=TaskId%></a>
					<td class="text-left"><%=TaskName%></td>
					<td class="text-left"><%=task_description%></td>					
					<td class="text-right"><%=Duration%></td>
					<td class="text-right"><%=StartDate%></td>
    ...
    {  	
		...
		<tr>
					...
					<td class="text-right"><%=UserID%></td>					
					<td style="font-size: 11px;" class="text-center">
					<a data-bs-toggle="modal" data-bs-target="#addMyModal" style="color: #0d6efd;cursor:pointer;" onclick="return demo('<%=datenew1%>','<%=datenew2%>','<%=HOD%>','<%=department%>','<%=User%>','<%=Status%>','<%=Role%>','<%=TaskName%>','<%=UserID%>','<%=TaskId%>')"><%=TaskId%></a>
 					<td class="text-left"><%=TaskName%></td>
					<td class="text-left"><%=task_description%></td>
					<td class="text-right"><%=Duration%></td>
					<td class="text-right"><%=StartDate%></td>
    ...
    //	query12="SELECT distinct(a.taskId) as TaskId,a.UserId,b.EmpName AS USER,a.TaskName,a.TaskDescription AS task_description,a.AssignedDuration AS Duration, a.TaskPercent,a.TaskComment AS TaskComment,a.TaskAttachment,DATE_FORMAT(a.TaskStartDate,'%d-%b-%Y %T' ) AS StartDate,DATE_FORMAT(a.TaskEndDate,'%d-%b-%Y %T') AS EndDate,a.TaskRejectionComment AS RejectionReason ,a.TaskStatus AS STATUS  FROM  db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"taskmaildataCopy  a JOIN db_GlobalERP.UserMaster b  ON a.UserID=b.dotprojectid  WHERE  b.empname LIKE '%"+name13+"%' AND b.companymasterid=100000 AND b.activestatus='Yes'  AND b.dept LIKE '%"+department+"%'  AND  a.TaskEndDate >='"+dt+"' AND a.TaskEndDate <='"+dt1+"' AND a.TaskStatus LIKE '%"+Status+"%' ORDER BY a.TaskID ";
		...
		while(rs.next())
		{	
			String TaskId="",TaskName="",Duration="",StartDate="",EndDate="",Stat="",TaskPercent="",TaskComment="",TaskAttachment="",task_description="",RejectionReason="";
			User=rs.getString("User");			
			UserID=rs.getString("UserID");			
			TaskAttachment=rs.getString("TaskAttachment");			
			String[] TA=TaskAttachment.split(",");		
...
function approve11()
{
	...
	var li=location.assign("OPTaskNextAdd.jsp?userId1="+userId+"&tskid1="+tskid1+"&comments311="+comments311+"&datefrom="+datefrom+"&dateto="+dateto+"&hod="+hod+"&dep="+dep+"&stst="+stst+"&usr="+usr+"&rol="+rol);
	alert("Loading...Please click OK");
	}
</script>
<script>
