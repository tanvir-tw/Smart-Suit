...
try {
		...
		String MM_dbConn_GPS = "jdbc:mysql://"+host1+"/dotproject";
...
if(datenew1==null){
	...
	System.out.println("==in else==");
	 ...
	 date2 = datenew2;
		...
		System.out.println("USERNAME on indexpge=>"+session.getAttribute("user"));
// 		String user=session.getAttribute("user");
// 			String usrid="select User_id from db_GlobalERP.dotprojectUserId where UserName="+session.getAttribute("user")+" ";
// 			ResultSet rst1=st4.executeQuery(usrid);
// 			while(rst1.next())
// 			{
// 				 user_id1= rst1.getString("User_id");
// 				 System.out.println("Userid111111111=>"+user_id1);
// 			}
...
//  	int cnt = 0;
//  	String sql_active = "select * from db_GlobalERP.dotprojectUserId where UserName='"+session.getAttribute("user")+"' ";
//  	ResultSet rs_active = st_user_id.executeQuery(sql_active);
//  	System.out.println("sql_active====>"+sql_active);
//  	while(rs_active.next()){
//  		user_id1 = rs_active.getString("user_id");
//  		System.out.println("user_id1====>"+user_id1);
//  		Dept=rs_active.getString("Dept");
//  		System.out.println("Dept====>"+Dept);
//  		HodReport=rs_active.getString("HodReport");
//  		System.out.println("HodReport====>"+HodReport);
//  		}
 		
//  	}
int cnt=0;
 	  if(session.getAttribute("user") == "Reshma" || session.getAttribute("user").equals("Reshma") )
 	  {
 		 System.out.println("HELOLOLOLO");
 		 String softw = "select User_id from db_GlobalERP.dotprojectUserId where  HodReport='Reshma Maner' ";
		 	ResultSet softwere = st10.executeQuery(softw);
		 	while(softwere.next())
		 	{ 
				...
				if(cnt == 1)
				{
					user_id1 = softwere.getString("user_id");
					System.out.println("user_id"+user_id1);
				}else
				{
					user_id1 = user_id1 +","+ softwere.getString("user_id");
				}
 	  ...
 	  else if(session.getAttribute("user") == "lalitha" || session.getAttribute("user").equals("lalitha") )
	  {
		 System.out.println("HELOLOLOLO");
		 String softw = "select User_id from db_GlobalERP.dotprojectUserId where  HodReport='Lalitha sankh' ";
		 	ResultSet softwere = st10.executeQuery(softw);
		 	while(softwere.next())
		 	{ 
...
<tr><td align="center"><font color="black" size="3"><b>Task Index Report Between <%= tDate1%> And <%=tDate2 %></b></font></td></tr>
<tr></tr>
<tr>
<td>
<br>
...
<table width="45%" border="0" align="right">
		<tr>
			<td>
							...
							<div style="text-align: right"><a href="#" style="font-weight: ; color: black; " onclick="gotoExcel('table1','tableHTML');">
                           <img src="images/excel.jpg" width="12px" height="12px" style="border-style: none"><%=new SimpleDateFormat("dd-MMM-yyyy HH:mm").format(new java.util.Date())%></img></a></div>
...
<tr >
<th bgcolor="#1582AB"><font size="2" face="Arial" color="white">Sr<br>No</font></th>
<th bgcolor="#1582AB"><font size="2" face="Arial" color="white">User<br>Name</font></th>
<th bgcolor="#1582AB"><font size="2" face="Arial" color="white">Total<br>Tasks</font></th>
<th bgcolor="#1582AB"><font size="2" face="Arial" color="white">A<br>Category</font></th>
<th bgcolor="#1582AB"><font size="2" face="Arial" color="white">B<br>Category</font></th>
<th bgcolor="#1582AB"><font size="2" face="Arial" color="white">C<br>Category</font></th>
<th bgcolor="#1582AB"><font size="2" face="Arial" color="white">D<br>Category</font></th>
<th bgcolor="#1582AB"><font size="2" face="Arial" color="white">A<br>Category %</font></th>
<th bgcolor="#1582AB"><font size="2" face="Arial" color="white">B<br>Category %</font></th>
<th bgcolor="#1582AB"><font size="2" face="Arial" color="white">C<br>Category %</font></th>
<th bgcolor="#1582AB"><font size="2" face="Arial" color="white">D<br>Category %</font></th>
<th bgcolor="#1582AB"><font size="2" face="Arial" color="white">Performance<br>Index %</font></th>
...
try{
	...
	if(session.getAttribute("user").equals("Reshma") && dept.equals("Software"))
	  {
 sql = "Select user_id, user_username from dotproject.users where user_id in ("+user_id1+") order by user_username ASC";
//  rs = st.executeQuery(sql);
System.out.println("The query is Hod==>>"+sql);
 	  
	  }
	
		 else if(session.getAttribute("user").equals("lalitha") && dept.equals("RnD"))
		  {
			 sql2 = "Select user_id, user_username from dotproject.users where user_id in ("+user_id1+") order by user_username ASC";
			//  rs = st.executeQuery(sql);
			System.out.println("The query is Hod==>>"+sql); 
	}
	...
	{
		   sql1 = "Select user_id, user_username from dotproject.users where user_id="+user_id1+" order by user_username ASC";
...
//Comenetd On 3 March  String sql_b = "select count(*) as b_cat from dotproject.tasks a, dotproject.user_tasks b where a.task_id=b.task_id and user_id='"+user_id+"' and date(task_end_date)>='"+fromDTime+"' AND date(task_end_date)<='"+toDTime+"' and a.task_percent_complete=100 and hour(timediff(a.task_completion_date_time,a.task_end_date))>24 and hour(timediff(a.task_completion_date_time,a.task_end_date))<=144 ";
		...
		//		System.out.println("The query is ==>>"+sql_b);
			...
			<%if(total_task > 0) {  %>
				<td bgcolor="#f5f5f5" align="right"><div align="right"><a href="showTotalTaskInIndexReport.jsp?totalTask=<%=(int)total_task%>&frmDate=<%= fromDTime%>&toTime=<%= toDTime%>&user_id=<%=user_id %>&user_name=<%= user_name%>"><%= (int)total_task%></a></div></td>
<%-- 				<a href="#" onclick="window.open('showTotalTaskInIndexReport.jsp?totalTask=<%=(int)total_task%>&frmDate=<%= fromDTime%>&toTime=<%= toDTime%>&user_id=<%=user_id %>&user_name=<%= user_name%>','mywindow','width=900, height=500, toolbar=false, location=false, status=no, menubar=no, resizable=no, scrollbars=yes,left=50, top=50 '); "><%= (int)total_task%></a></div></td> --%>
			<%} else {%>
				<td bgcolor="#f5f5f5" align="right"><div align="right"><%= (int)total_task%></div></td>
			<%} %>
			<%if(a_cat > 0) {  %>
				<td bgcolor="#f5f5f5" align="right"><div align="right"><a href="showAcatTaskInIndexReport.jsp?AcatTask=<%=(int)a_cat%>&frmDate=<%= fromDTime%>&toTime=<%= toDTime%>&user_id=<%=user_id %>&user_name=<%= user_name%>"><%= (int)a_cat%></a></div></td>
			<%} else {%>
				<td bgcolor="#f5f5f5" align="right"><div align="right"><%= (int)a_cat%></div></td>
			<%} %>
			<%if(b_cat > 0) {  %>
				<td bgcolor="#f5f5f5" align="right"><div align="right"><a href="showBcatTaskInIndexReport.jsp?BcatTask=<%=(int)b_cat%>&frmDate=<%= fromDTime%>&toTime=<%= toDTime%>&user_id=<%=user_id %>&user_name=<%= user_name%> "><%= (int)b_cat%></a></div></td>
			<%} else {%>
				<td bgcolor="#f5f5f5" align="right"><div align="right"><%= (int)b_cat%></div></td>
			<%} %>
			<%if(c_cat > 0) {  %>
				<td bgcolor="#f5f5f5" align="right"><div align="right"><a href="showCcatTaskInIndexReport.jsp?CcatTask=<%=(int)c_cat%>&frmDate=<%= fromDTime%>&toTime=<%= toDTime%>&user_id=<%=user_id %>&user_name=<%= user_name%>"><%= (int)c_cat%></a></div></td>
			<%} else {%>
				<td bgcolor="#f5f5f5" align="right"><div align="right"><%= (int)c_cat%></div></td>
			<%} %>
			<%if(d_cat > 0) {  %>
				<td bgcolor="#f5f5f5" align="right"><div align="right"><a href="showDcatTaskInIndexReport.jsp?DcatTask=<%=(int)d_cat%>&frmDate=<%= fromDTime%>&toTime=<%= toDTime%>&user_id=<%=user_id %>&user_name=<%= user_name%>"><%= (int)d_cat%></a></div></td>
			<%} else {%>
