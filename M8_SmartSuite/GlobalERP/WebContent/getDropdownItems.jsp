...
/* 	 String buffer="<select name='makeList' id='makeList' style='width: 150px; height:20px;  "+
	...
	/* String buffer="<select name='makeList' id='makeList' style='width: 150px; height:20px;  "+
			 " border: 1px solid black; font: normal 11px Arial, Helvetica, sans-serif;   "+
			 " color: #000000; ' onchange='OnChangeGetDetails(this,\"make\");'><option value='Select' >Select</option>";  */
			 String buffer = "<label class='tcolor'>Make</label>" +
			    "<select class='form-select' name='makeList' id='makeList' " +
			    "onchange='OnChangeGetDetails(this, \"make\");' required>" +
	...
	else
	{
	 ...
	 sql1="SELECT distinct(MakeName) from "+tableName.toLowerCase()+" group by MakeName";
		   ...
		   {
			   flag=true;
		      String sqlcode="SELECT distinct(MakeName) from "+tableName.toLowerCase()+" order by MakeName asc";
...
else if(request.getParameter("make")!=null)
{
	...
	String buffer =  "<label class='tcolor'>Model</label>" +
		    "<select class='form-select' name='modelList' id='modelList' " +
		    "onchange='OnChangeGetDetails(this, \"model\");' required>" +
		    "<option value='Select'>Select</option>";
...
else if(request.getParameter("model")!=null)
{
	...
	String buffer=
	 "<label class='tcolor'>Description</label>" +
	    "<select class='form-select' name='DescriptionList' id='DescriptionList' " +
	    "onchange='OnChangeGetDetails(this, \"description\");' required>" +
	    "<option value='Select'>Select</option>";
	 ...
	 int i=1;
	   ...
	   buffer+="<label class='tcolor'>Part No</label>" +
			    "<select class='form-select' name='partNoList' id='partNoList' " +
			    "onchange='OnChangeGetDetails(this, \"partNo\");' required>" +
			    "<option value='Select'>Select</option>";
...
String b=model[0];
	...
	String buffer="<select name='DescriptionList' id='DescriptionList' style='width: 300px; height:20px;  "+
	 " border: 1px solid black; font: normal 11px Arial, Helvetica, sans-serif;   "+
	 " color: #000000; ' onchange='OnChangeGetDetails(this,\"description\");'><option value='Select' >Select</option>"; 
	 int i=1;
