...
%>
	...
	Statement st1=conn1.createStatement();
		...
		//Connection conn = null;
			...
			String fromDateCal="",toDateCal="",fromdatevdTime="",todatevdTime="";
			String sqlDetails="";
	...
	//Date monthsDateAsDate = cal.getTime();
	 Calendar date = Calendar.getInstance();
      ...
      DateFormat df23= new SimpleDateFormat("dd/MM/yyyy");
       String startDate=df23.format(date.getTime());
...
try 
{
	...
	//String DB_NAME="jdbc:mysql://103.8.126.138:3306/db_gps";
	//String DB_NAME="jdbc:mysql://192.168.2.55:3306/db_gps";
	//String DB_NAME123="jdbc:mysql:// twdb.fleetview.in/db_gps";
	//String DB_NAME1="jdbc:mysql://103.8.126.138:3306/twsql_twalldata";
	//String DB_NAME1="jdbc:mysql://192.168.2.55:3306/twsql_twalldata";
	//String DB_NAME11234="jdbc:mysql:// twdb.fleetview.in/db_GlobalERP";
	//String DB_USERNAME1="erp";
	// String DB_PASSWORD1="1@erp";
	Class.forName("com.mysql.cj.jdbc.Driver");
	//conn=DriverManager.getConnection(DB_NAME,DB_USERNAME,DB_PASSWORD);
	conn1=DriverManager.getConnection(DB_NAME11234,DB_USERNAME1,DB_PASSWORD1);
	sterp = conn1.createStatement();
	...
	// Statement st1=conn1.createStatement();
				...
				stMaster= conn1.createStatement();
					stMaster1=conn1.createStatement();
...
if(f_day.equals("01")){
	...
	Date d2 = new SimpleDateFormat("yyyy-MM-dd").parse(fromDate);
	Calendar cal12= Calendar.getInstance();
	cal12.setTime(d2);
	 d3 = cal12.getActualMaximum(Calendar.DAY_OF_MONTH);	
	...
	if((Integer.parseInt(l_day) == d3) && (d3 == days_of_month))
	{
		//For Monthly index
		flag_day = true;
	}			
}
System.out.println("The d1 in month ===================>"+d3);
...
<form id="marketingreport" name="marketingreport" method="post">	
 ...
 <br></br>
	...
	<%
		String sqlMarktRep="Select *  from 100000marketingrepmaster as tm,100000security as ts WHERE  tm.username<>'' and tm.password<>'' and (tm.Compressor=1 or tm.DigitalEquipment=1 or tm.DGSet=1 or tm.Endoscope=1 or tm.OtherGroups=1) and ts.activestatus='Yes' and tm.username = ts.username  order by  tm.MarketingRepNAme";
...
String sqlRep="Select *  from 100000marketingrepmaster as tm,100000security as ts WHERE  tm.username<>'' and tm.password<>'' and (tm.Compressor=1 or tm.DigitalEquipment=1 or tm.DGSet=1 or tm.Endoscope=1 or tm.OtherGroups=1) and ts.activestatus='Yes' and tm.username = ts.username  order by  tm.MarketingRepNAme";
...
{			
	System.out.println("QRYYYYc For  Marketing Rep**======>+i"+i);
		...
		<%
			...
			ResultSet rsCust=st3.executeQuery(sqlCustomers);
			if(rsCust.next())
...
%>	
		...
		<td align="right"> <a href="#" onclick="popUp('customerFollowupList.jsp','<%=code %>','<%=CountMain %>','<%= 1%>');"><%=CountMain %></a></td>
		<%
...
ResultSet rsList=st3.executeQuery(sqlList);
System.out.println(sqlList);
		...
		<td align="right"><a href="#" onclick="popUpNew('customerFollowupsecondlevel.jsp','<%=code %>','<%=totcount %>','<%=2 %>','<%=toDate %>','<%=fromDate %>');"><%=totcount %></a></div></td>
		...
		String missedinlastmonth="SELECT * FROM db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"followup Group By ProspCustName having max(NextFollowUpDate)  Between '"+fromDate+"' AND '"+toDate+"' and MarketingRepcode = '"+code+"'";
		 ...
		 ResultSet rsMissedmonth=st3.executeQuery(missedinlastmonth);
		...
		<td align="right"><a href="#" onclick="popUpNew('customerFollowupsecondlevel.jsp','<%=code %>','<%=count%>','<%=3 %>','<%=toDate %>','<%=fromDate %>');"><%=count%></a></div></td>
	...
	//	String Folllastyesterday="SELECT * FROM db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"followup Group By ProspCustName having FollowUpDate = '"+yesterdaysDate+"'  and MarketingRepcode = '"+code+"'";
		...
		String Folllastyesterday="select * from "+session.getAttribute("CompanyMasterID").toString()+"followup where MarketingRepcode='"+code+"' AND ProspCustName <> '-'  AND  FollowUpDate = '"+yesterdaysDate+"' ";
		 ...
		 ResultSet rsyesterday=st3.executeQuery(Folllastyesterday);
		...
		<td align="right"><a href="#" onclick="popUpNew('customerFollowupsecondlevel.jsp','<%=code %>','<%=countFollNew%>','<%=4%>','<%=yesterdaysDate %>','<%=yesterdaysDate %>');"><%=countFollNew%></a></div></td>
		<%
		...
		String missedinlastyesterday="SELECT * FROM db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"followup Group By ProspCustName having max(NextFollowUpDate)='"+yesterdaysDate+"'  and MarketingRepcode = '"+code+"'";
		 ...
		 ResultSet rsMissedyesterday=st3.executeQuery(missedinlastyesterday);
		 ...
		 %>
		<td align="right"><a href="#" onclick="popUpNew('customerFollowupsecondlevel.jsp','<%=code %>','<%=countNew%>','<%=5%>','<%=yesterdaysDate %>','<%=yesterdaysDate %>');"><%=countNew%></a></div></td>
		...
		String Todayqry="select ProspCustName,ContactPerson,Address,City,ProspCustCode from "+session.getAttribute("CompanyMasterID").toString()+"followup where MarketingRepcode='"+code+"' AND ProspCustName <> '-'  AND  NextFollowUpDate = '"+todaysDate+"' ";
		 ...
		 ResultSet rstoday=st3.executeQuery(Todayqry);
		 while(rstoday.next())
		 {
		 ...
		 System.out.println("Missed Foll Cont===>"+countNew);
		 %>
		<td align="right"><a href="#" onclick="popUpNew('customerFollowupsecondlevel.jsp','<%=code %>','<%=CountToDayNew%>','<%=6%>','<%=todaysDate %>','<%=todaysDate %>');"><%=CountToDayNew%></a></div></td>
		...
		String Tomorqry="select ProspCustName,ContactPerson,Address,City,ProspCustCode from "+session.getAttribute("CompanyMasterID").toString()+"followup where MarketingRepcode='"+code+"' AND ProspCustName <> '-'  AND  NextFollowUpDate = '"+tomoDate+"'";
		 ...
		 ResultSet rstom=st3.executeQuery(Tomorqry);
		 while(rstom.next())
		 {
		 ...
		 %>
		<td align="right"><a href="#" onclick="popUpNew('customerFollowupsecondlevel.jsp','<%=code %>','<%=CountTomorNew%>','<%=7%>','<%=tomoDate %>','<%=tomoDate %>');"><%=CountTomorNew%></a></div></td>
