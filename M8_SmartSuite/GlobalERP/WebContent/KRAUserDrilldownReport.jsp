...
<div class="content-header-col-md-7-mt-10 col-12"> <!-- Added text-center class -->
    <div class="row breadcrumbs-top">
      <h5 class="content-header-title text-center mb-0" style="color: #060637">KRA Summary for <%=projectName%> From <%=datex1%> To <%=datex2%></h5>
    </div>
</div>
...
ResultSet rsmain = st4.executeQuery(sqlmain);
while(rsmain.next()){
...
taskid=rsmain.getString("taskid");	
  ...
  if(rs1.next()){
	  ...
	  String sql11="select EmpName,Dept from db_GlobalERP.UserMaster where KRAdotprojectid='"+UserId+"'";
	  System.out.println("sql11----->"+sql11);
	  ResultSet rs11 = st5.executeQuery(sql11);
	  if(rs11.next()){
	  ...
	  if(MailDateTime.equalsIgnoreCase("-")){
    	 ...
    	 String sql2= "select * from dotproject.tasks where task_id='"+taskid+"' and task_end_date>='"+startDate+" 00:00:00' and task_end_date <='"+endDate+" 23:59:59'";
    	  ...
    	  if(rs2.next()){
    		  ...
    		  String sql22="select EmpName,Dept from db_GlobalERP.UserMaster where KRAdotprojectid='"+userId+"'";
    		  System.out.println("sql22----->"+sql22);
    		  ResultSet rs22 = st3.executeQuery(sql22);
    		  if(rs22.next()){
					...
					buttons : [
							{
	                         ...
	                         title: 'KRA Summary From <%=datex1%> To <%=datex2%>', 
							},
							{
							 extend: 'pdfHtml5',
