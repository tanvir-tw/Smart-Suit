...
int total_task=0,total_task1=0;
...
Statement stDepartment1=null,stDepartment=null;

try {
	con1 = connection.getConnect("db_GlobalERP");    
	st=con1.createStatement();
	stDepartment=con1.createStatement();
	stDepartment1=con1.createStatement();
...
<div class="modal fade" id="addTaskModal" tabindex="-1" aria-labelledby="#addMyModal" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content" id="content">
       <form name="form1" id="AddTaskform" method="post" action="OPinsertTask.jsp">
    ...
    <div class="col-md-4">
      <label for="tsk1" class="control-label mb-0"><b>Department:</b></label>
				  ...
				  <div class="col-md-12 d-flex align-items-center">
				    ...
				    <div class="col-md-6">
				      <input type="text" class="form-control" id="ProjectId" name="ProjectId" style="border: 1px solid #ced4da;" onblur="AjaxValidateMembersipPI();">   
				    </div>
				     <div class="col-md-4"></div>
				  </div>
				</div>
...
<div class="modal fade" id="addKRAModal" tabindex="-1" aria-labelledby="#addMyModal" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content" id="content">
             <form name="form1" id="AddKRAform" method="post" action="OPinsertTask.jsp" >    
       <div class="modal-body">
...
<div class="row mt-1">
  <div class="col-md-12 d-flex align-items-center">
    ...
    <select class="form-select" id="Department1" name="Department1"  onchange="concernpersonName1();" style="font-size: 0.8rem;" >    
								<option selected>Select Department</option>															
								<option value="All">All</option>
								<%String sqlDepartment1="select DepartmentName from db_GlobalERP."+comid+"DepartmentMaster";
 ...
 <% String person1=request.getParameter("concernperson");
  ...
  <div class="col-md-12 d-flex align-items-center">
    ...
    <div class="col-md-6">
      <input type="text" class="form-control" id="ProjectId1" name="ProjectId1" style="border: 1px solid #ced4da;" onblur="AjaxValidateMembersipKRA();">   
    </div>
     <div class="col-md-4"></div>
  </div>
</div>
...
<table id="example" class="display" style="width:100%;">        
       <thead>
        ...
        <tbody>
         ...
         String sqlTaskAllocation = "select Empname as User,dotprojectid as UserID,kradotprojectid as KRAid,Openprojectid,empcode,HodReport as HOD,Dept as Department from db_GlobalERP.UserMaster  where activestatus='Yes'  and companymasterid="+session.getAttribute("CompanyMasterID").toString()+" and dept like '%"+DepartmentAll+"%' and  (HODReport like '%"+HODAll+"%' OR EmpName like '%"+EmpNameAll+"%' ) and Openprojectid!='-'  order by Empname";
	      ...
	      while(rs.next()){
	    	  ...
	    	  //PI -work_packages
	    		...
	    		String sql_total = "select sum(estimated_hours) as total_tasks from work_packages where assigned_to_id='"+OpenProjectId+"' and due_date >='"+dt+"' and due_date<='"+dt1+"' and type_id=1 ";
			...
			int	AssignedNew = total_task;
	    	  System.out.println("Records :- "+EmpCode+" , "+UserName+" , "+HOD+" , "+Dept+" , "+AssignedNew+" , "+KRAid+" ");
	    	  
	    	  //KRA -work_packages
	    	  String sql_total1 = "select sum(estimated_hours) as total_tasks from work_packages where assigned_to_id='"+OpenProjectId+"' and due_date >='"+dt+"' and due_date<='"+dt1+"' and type_id=9 ";
				//System.out.println("The query is ==>>"+sql_total);
				System.out.println("The query is ==>>"+sql_total1);
				ResultSet rst1 = stmt2.executeQuery(sql_total1);
				
				if(rst1.next()){
			  		total_task1 = rst1.getInt("total_tasks");
				}
				int AssignedNewKRA = total_task1;
         ...
         <tr> 
            	...
            	<td class="text-left"><%=Dept%></td>
            	 ...
            	 <a href="OPTaskDetRepFrTaskAllo.jsp?userid=<%=OpenProjectId %>&fromdate=<%=datex1 %>&todate=<%=datex2 %>&HOD=<%=HOD%>&Dept=<%=Dept %>&User=<%=UserName %>&Status=Assigned&role=<%=Sesrole2%>">
            	 <%=AssignedNew%></a></td>           	           	
            	 <td  class="text-center">
            	<a href="OPKRADetRepFrTaskAllo.jsp?userid=<%=OpenProjectId %>&fromdate=<%=datex1 %>&todate=<%=datex2 %>&HOD=<%=HOD%>&Dept=<%=Dept %>&User=<%=UserName %>&Status=Assigned&role=<%=Sesrole2%>">
            	 <%=AssignedNewKRA%></a></td>
				</tr>
				<%
                   i++;      
...
%>
 ...
 <script>
  ...
  function userid()
  {
  	...
  	ajaxRequest.open("GET", "OPUserIdAjax.jsp"+queryString, true);
  	ajaxRequest.send(null);  	
  	}
  function userid1()
  {
  ...
  <script>
function AjaxValidateMembersipPI() {

  //  alert("Hi in validate project membership");

    var proj = document.getElementById('ProjectId').value;
    var user = document.getElementById('userId').value;

   // alert("projectid:" + proj + " user:" + user);
    ...
    ajaxRequest.onreadystatechange = () => { // Corrected arrow function syntax
        if (ajaxRequest.readyState == 4 && ajaxRequest.status == 200) { // Check for status 200 (OK)
            var reslt = ajaxRequest.responseText;
            var  result = reslt.split("#");

            if (result[0].trim() === "false" || result[0].trim() === "0") {  // Compare with string values, trim for whitespace
                alert(result[1]);
                document.getElementById('userId').value = "";
                document.getElementById('ProjectId').value = "";
               
            } else if(result[0].trim() === "true" || result[0].trim() === "1"){
                 // Optionally you can add a success message or perform other actions
                 alert("User Validated with ProjectId You can Proceed with Task Allocation")
            } else {
                // Handle other cases like errors from the server
                alert("An error occurred during validation. Response: " + reslt);
            }

        } else if (ajaxRequest.readyState == 4 && ajaxRequest.status != 200) {
            alert("Error: " + ajaxRequest.status); // Handle HTTP errors
        }
    };

    var URL = "Ajax ValidateMembership.jsp?userid=" + user + "&projectid=" + proj;
...
function AjaxValidateMembersipKRA() {

    //alert("Hi in validate project membership");

    var proj = document.getElementById('ProjectId1').value;
    var user = document.getElementById('userId1').value;
        ...
        if (ajaxRequest.readyState == 4 && ajaxRequest.status == 200) { // Check for status 200 (OK)
            ...
            if (result[0].trim() === "false" || result[0].trim() === "0") {  // Compare with string values, trim for whitespace
                ...
                document.getElementById('ProjectId').value = "";
                 ...
                 // Optionally you can add a success message or perform other actions
                 alert("User Validated with ProjectId You can Proceed with KRA Task Allocation")
            } else {
                // Handle other cases like errors from the server
                alert("An error occurred during validation. Response: " + reslt);
            }
