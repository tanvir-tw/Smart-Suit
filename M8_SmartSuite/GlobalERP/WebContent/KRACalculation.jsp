...
String datenew1="",datenew2="",date1 = "", date2 = "", dept = "",user_id1 = "",tDate1 = "", tDate2 = "", UserName = "";
String sql_active = "";
int k=2000;
int cnt = 0;
 // Connection Initialization
 Statement st = null,st1 = null,stt = null,sta = null, stb = null,stc = null,std = null,st_up = null,st_up1 = null,stmt =null,stmt1=null;
Statement st_user_id = null,st2 = null,st3 = null,st4 = null,st5 = null, st6 = null,st7 = null, st8 = null, st9 = null, st10=null;
...
try{
if (!companyid.equals("100000")) {
    total_task1 = 100;
    System.out.println("in companyid other than 100000");
}
...
<div class="content-header-col-md-7-mt-10 col-12"> 
  ...
  <form name="indexReport" id="indexReport" method="get" onsubmit="return allValidation()" action="">
...
<%
    ...
    // If no month selected, use previous month
    if (selectedMonth == null || selectedMonth.isEmpty() || selectedMonth.equals("Select")) {
        java.time.LocalDate now = java.time.LocalDate.now();
        java.time.LocalDate prevMonthDate = now.minusMonths(1);

        selectedMonth = String.format("%02d", prevMonthDate.getMonthValue()); // e.g. "03"
    }
...
%>
    <option value="<%=value%>" <%=selectedmon%>>
        <%=label%>
    </option>
<%
    }
...
<table id="example" class="display" style="width:100%">        
       ...
       while(rs.next()) {
       		...
       		String Data1 = "select EmpName from db_GlobalERP.UserMaster where companymasterid='"+session.getAttribute("CompanyMasterID").toString()+"' and (dotprojectid='"+user_id+"' or KRAdotprojectid='"+user_id+"')";
       		System.out.println("The query for emp name ==>>"+Data1);
       		ResultSet rsUser = st10.executeQuery(Data1); 
       	    ...
       	    while (rsUser.next()) {
       	         UserName = rsUser.getString("EmpName");
       				...
       				if (!companyid.equals("100000")) {
       					String sql_a="select sum(a.task_duration) as a_cat from dotproject.tasks a,  dotproject.user_tasks b where a.task_id=b.task_id and  user_id='"+user_id+"' and date(task_end_date)>='"+fromDTime+"' AND  date(task_end_date)<='"+toDTime+"'  and a.task_percent_complete=100 and  a.task_completion_date_time >  a.task_end_date and DATE(a.task_completion_date_time) <= (SELECT DATE(date(a.task_end_date) + INTERVAL (5 - WEEKDAY(date(a.task_end_date))) DAY))";
       					...
       					String sql_b="select sum(a.task_duration) as a_cat from dotproject.tasks a,dotproject.user_tasks b where a.task_id=b.task_id and user_id='"+user_id+"' and date(task_end_date) >='"+fromDTime+"' AND  date(task_end_date) <='"+toDTime+"' and a.task_percent_complete=100 and  a.task_completion_date_time >  a.task_end_date and DATE(a.task_completion_date_time) <= (SELECT DATE(date(a.task_end_date) + INTERVAL (5 - WEEKDAY(date(a.task_end_date))) DAY))";
       					ResultSet rsb = stb.executeQuery(sql_b);
       					System.out.println("The query is ==>>"+sql_b);
       					if(rsb.next()){
       					
       						a_cat = rsb.getDouble("a_cat");
       					}
       					//calculate the c cat task
       					
       					String sql_c="select sum(a.task_duration) as a_cat from dotproject.tasks a,dotproject.user_tasks b where a.task_id=b.task_id and user_id='"+user_id+"' and date(task_end_date) >='"+fromDTime+"' AND  date(task_end_date) <='"+toDTime+"' and a.task_percent_complete=100 and  a.task_completion_date_time >  a.task_end_date and DATE(a.task_completion_date_time) <= (SELECT DATE(date(a.task_end_date) + INTERVAL (5 - WEEKDAY(date(a.task_end_date))) DAY))";
       					//System.out.println("The query is ==>>"+sql_c);
       					ResultSet rsc = stc.executeQuery(sql_c);
       					System.out.println("The query is ==>>"+sql_c);
       					if(rsc.next()){
       					
       						a_cat = rsc.getDouble("a_cat");
       					}
       					...
       					System.out.println("The query is ==>>"+sql_d);				
       					System.out.println("The query is ==>>"+df.format(c_cat)+" "+df.format(a_cat)+" "+df.format(b_cat));
       			//end of if condition		
       				}else {
       				//calculate the a cat task
       				String sql_a="select sum(task_duration) as a_cat from dotproject.tasks a,  dotproject.user_tasks b where a.task_id=b.task_id and  user_id='"+user_id+"' and date(task_end_date)>='"+fromDTime+"' AND  date(task_end_date)<='"+toDTime+"'  and a.task_percent_complete=100 and  a.task_completion_date_time <= a.task_end_date";
       				...
       				String sql_b="select sum(a.task_duration) as b_cat from dotproject.tasks a,dotproject.user_tasks b where a.task_id=b.task_id and user_id='"+user_id+"' and date(task_end_date) >='"+fromDTime+"' AND  date(task_end_date) <='"+toDTime+"' and a.task_percent_complete=100 and  a.task_completion_date_time >  a.task_end_date and DATE(a.task_completion_date_time) <= (SELECT DATE(date(a.task_end_date) + INTERVAL (5 - WEEKDAY(date(a.task_end_date))) DAY))";
       				ResultSet rsb = stb.executeQuery(sql_b);
       				System.out.println("The query is ==>>"+sql_b);
       				if(rsb.next()){
       				...
       				String sql_c="select sum(a.task_duration) as c_cat from dotproject.tasks a,dotproject.user_tasks b where a.task_id=b.task_id and user_id='"+user_id+"' and date(task_end_date) >='"+fromDTime+"' AND  date(task_end_date) <='"+toDTime+"' and a.task_percent_complete=100 and  a.task_completion_date_time >  a.task_end_date and DATE(a.task_completion_date_time) > (SELECT DATE(date(a.task_end_date) + INTERVAL (5 - WEEKDAY(date(a.task_end_date))) DAY)) and date(a.task_completion_date_time) <=(SELECT DATE(LAST_DAY(date(a.task_end_date)) +INTERVAL 4 Day))";
       				ResultSet rsc = stc.executeQuery(sql_c);
       				System.out.println("The query is ==>>"+sql_c);
       				if(rsc.next()){
       				...
       				double days=0.00;
       				System.out.println("Days :- "+totdays);
       				while(rstotdays.next()){
       					...
       					System.out.println("Month123 :- "+mnth);
    					...
    					else
    					{
    						...
    						}else
    						{
    						 ...
    						 else
    						 {
    							 ...
    							 {
    							 System.out.println("Inside Elsessssssssssssssssssssssssss ");
    				...
    				System.out.println("Total Leave Days  leaveday2 :- "+leaveday2);				
    				double  presentdays=days-leaveday2;
    				
    				System.out.println("Total Days hours :- "+total_task1);
    				if (!companyid.equals("100000")) {
    					 if(total_task >0) {
    						 
    						 a_cat_per = (a_cat * 100 / total_task1);
    					
    						 double data = ((a_cat/2) * 100 / total_task1);
    						 System.out.println("in if !100000 dataaaaaaaaaa  :"+data);
    						 
    					 }else{
    						 
    						 a_cat_per = 0.0;
    						 System.out.println("else  :"+a_cat_per);
    					 }
    					
    					 if(total_task >0) {
    						 
    						 b_cat_per = (b_cat * 100 / total_task1);
    					 }else{
    					...
    					if(total_task >0) {
    						 ...
    						 c_cat_per = (c_cat * 100 / total_task1);
    					 }else{
    					...
    					double per_index1 = a_cat_per + b_cat_per + c_cat_per + d_cat_per;
    					System.out.println("per_index111     ==>>"+per_index1);
    				}
    				 ...
    				 if(total_task >0) {
    					 ...
    					 a_cat_per = (a_cat * 100 / total_task1);
    					 System.out.println("if(total_task >0)))))))))a_cat   "+a_cat );
    					 System.out.println("if(total_task >0)))))))))total_task1  "+total_task1);
    					 System.out.println("if(total_task >0)))))))))  "+a_cat_per);
    					 double data = ((a_cat/2) * 100 / total_task1);
    				...
    				System.out.println("per_index     ==>>"+per_index);

    				todaysDate = new SimpleDateFormat("yyyy-MM-dd").format(new java.util.Date());
         ...
         <tr> 
				...
				<td class="text-right"><%= df.format(days)%></td>
				<%if(total_task > 0) {  %>
			...
			<td class="text-center"><%=df.format(per_index)  %></td>
				...
				if (Sesrole1.equals("Hr"))
				{
					...
					if(!rs21.next())
					{
						System.out.println("INSIDE VERSION IF");
						version="0";
					}
					else
					{
						System.out.println("INSIDE VERSION else");
					...
					System.out.println("Depart is::::"+department1);
