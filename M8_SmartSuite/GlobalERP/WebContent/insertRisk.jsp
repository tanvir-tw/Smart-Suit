...
try
	{
		...
		String sql="select * from "+session.getAttribute("CompanyMasterID").toString()+"riskInvolvedMaster where riskInvolved='"+risk+"' ";
		...
		if(rs1.next()){
			...
			<jsp:forward page="alertGoTo.jsp">
			 <jsp:param name="msg" value="This Risk Involved Is Already Exist  " ></jsp:param>
			  <jsp:param name="goto" value="AddNewRisk.jsp" ></jsp:param>
		...
		String compQuery="insert into "+session.getAttribute("CompanyMasterID").toString()+"riskInvolvedMaster (riskInvolved, description, EntryBy,status) values (?,?,?,?)";
		...
		response.sendRedirect("alertGoTo.jsp?msg=New Risk Added Successfully&goto=riskInvMaster.jsp");
		}
}
catch(Exception e)
{
