...
bt=request.getParameter("button");
 ...
 if(datenew1==null){
	...
	if(name.equals("Arati Kansagara") || name.equals("Deepa khutwad"))
	{
		name="Vikram Puri";
	}
...
<table id="example" class="display" style="width:100%">        
	...
	else if(IdexType.equals("KRA"))
	{
		...
		if(dept1.equals("All"))
		{
			sql_total="select distinct (a.TaskID) as TaskID ,a.TaskName , a.TaskCreater ,a.TaskComment,a.TaskStartDate,a.AssignedDuration,a.TaskEndDate,a.MailDateTime,a.UserDepartment,a.UserId,b.EmpName,a.TaskStatus,a.TaskAttachment,a.TaskPercent,a.TaskDescription from db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"taskmaildataCopy a , db_GlobalERP.UserMaster b  where date(a.taskenddate) >='"+fromDTime+"' AND date(a.taskenddate) <='"+toDTime+"' and a.TaskStatus='Pending'  and b.dept='"+dept1+"'  and a.UserId="+dotprojectid+"  and b.HODReport='"+name+"'  and b.ActiveStatus='Yes' order by UserId,TaskStartDate ";
		}
		else
		{
			if(EmpName.equals("All")){
				...
				sql_total="select distinct (a.TaskID) as TaskID ,a.TaskName , a.TaskCreater ,a.TaskComment,a.TaskStartDate,a.AssignedDuration,a.TaskEndDate,a.MailDateTime,a.UserDepartment,a.UserId,b.EmpName,a.TaskStatus,a.TaskAttachment,a.TaskPercent,a.TaskDescription from db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"taskmaildataCopy a , db_GlobalERP.UserMaster b  where date(a.taskenddate) >='"+fromDTime+"' AND date(a.taskenddate) <='"+toDTime+"' and a.TaskStatus='Pending'  and b.dept='"+dept1+"' and b.EmpName='"+EmpName+"'  and a.UserId="+dotprojectid+"  and b.HODReport='"+name+"'  and b.ActiveStatus='Yes' order by UserId,TaskStartDate ";
			}
		}
		...
		while(rs.next()){
	        ...
	        task_description = rs.getString("a.TaskDescription");
	        	 ...
	        	 'colvis',
				{
					...
					buttons : [
	                         ...
	                         title: 'Task Approval Report from '+$("#data").val()+ 'to' +$("#data1").val(),							},
							{
							 ...
							 className: 'dropdown-item',
							 title: 'Task Approval Report from '+$("#data").val()+ 'to' +$("#data1").val(),	                            orientation: 'landscape',
	                            pageSize: 'A2',
	                            titleAttr: 'PDF'
							},
							{
...
function DeleteRow(id,indxtype,dept,ename,frdate,todate)
{
	var agree=confirm("Do You Want To Approve Task ?");
	    if(agree)
	    {	    	
	    	window.location="OPActionApprovedTask.jsp?action=approve&id="+id+"&indxtype="+indxtype+"&dept="+dept+"&ename="+ename+"&frdate="+frdate+"&todate="+todate+"";
	    }
	    else
	    {
	    	return false;
...
function getcheckedrow1(indxtype,dept,ename,frdate,todate)
{
	...
	if(flag == "success")
	{
		...
		DeleteRow1(id,cnt,indxtype,dept,ename,frdate,todate);//id,indxtype,dept,ename,frdate,todate
	}
	else
	{
		alert("Please Check Task Id ");
