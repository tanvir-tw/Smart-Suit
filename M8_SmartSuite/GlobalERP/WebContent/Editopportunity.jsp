...
<link rel="stylesheet" href="cssnew/newentryform.css" />
 ...
 <body style="background-color:azure;" onkeypress="hideList(event);" onclick="hideOnClick();">
...
%>
 ...
 String opportunitycount=request.getParameter("opportunitycount");
 String assignto=request.getParameter("assignto");
 String leadRef=request.getParameter("leadRef");
 String product=request.getParameter("product");
 String potentialValue=request.getParameter("potentialValue");
 String noofunits=request.getParameter("noofunits");
 String count=request.getParameter("count");
 String ooc=request.getParameter("Opportunitycode");
 String Category=request.getParameter("custtype");
 String CustomerCode="";
 String companyname=request.getParameter("companyname");
 companyname=companyname.replace("...", "&");
 String Custcode=request.getParameter("customercode");
 String getCode="",ccode="";
 if(Category.equals("Customer"))
 {
 getCode="select * from db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"prospectivecustmaster where CompanyName='"+companyname.replace("...", "&")+"'";
 System.out.println("To Get Orignal Code"+getCode);
 ResultSet rs1=st5.executeQuery(getCode);
 if(rs1.next())
 {
 	ccode=rs1.getString("SalesCustomerCode");
 }
 }
 else
 {
	getCode="select * from db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"prospectivecustmaster where CompanyName='"+companyname.replace("...", "&")+"'";
 	...
 	if(rs1.next())
 	{
 		ccode=rs1.getString("CustomerCode");
 	}
 	
 }
if(Category.equals("Prospect"))
 {
 	String OriginalCompany="select * from  db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"prospectivecustmaster where CompanyName='"+companyname+"' and CustomerCode='"+Custcode+"' ";
 	ResultSet rs2 = st3.executeQuery(OriginalCompany);
 ...
 else
 {
 	...
 	String OriginalCompany1="select * from  db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"prospectivecustmaster where SalesCustomerCode='"+Custcode+"' ";
 	ResultSet rs2 = st4.executeQuery(OriginalCompany1);
 	while (rs2.next()) {
 	CustomerCode=rs2.getString("CustomerCode");
 	}
 }
            ...
            <div class="card" style="background-color: transparent;">
                <div class="card-body">
		            <div class="row breadcrumbs-top">
		          		<h5 class="content-header-title text-center mb-0" style="color: #060637"><b>Edit Opportunity For <%=companyname%></b></h5>
		    		</div>
		             <br>
  <form name="addCustomer" action="Updateopportunity.jsp" method="post" onsubmit="return formValidation();">
  ...
  <div class="card-body">
      <div class="row col-12 mt-4"> 
										 ...
										 <div class="col-3">
												<div class="input-container">
													...
													<input type="hidden" name="comp" id="comp" value="<%=companyname%>"></label>
												</div>
												</div>
											...
											<div class="col-3  mt-4">
												<div class="input-container">
													<input class="form-control floating-input" type="text"
														id="contactperson@123" name="contactPerson" value="<%=contactperson%>"  onblur="return contactpersonfun();" placeholder=""
											...
											<div class="col-3 mt-4">
												<div class="input-container">
													<input id="website" name="website"
														class="form-control floating-input" value="<%=website%>"
	...
	String sql = "select MarketingRepName ,MarketingRepCode from db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"marketingrepmaster "+
		...
		%>
					...
					<input type="hidden" name="assignedBy"class="form-control floating-input" value="<%=session.getAttribute("EmpName").toString() %>"  />
					<input type="hidden" name="ooporcode"  class="form-control floating-input" value="<%=ooc%>"  />						
...
<div class="col-3 mt-4 ">
							<div class="input-container">
<input id="leadRef" name="leadRef"  class="form-control floating-input" type="text" placeholder="" value="<%=leadRef%>" onblur="return contactpersonfun11();">
      <label for="leadRef" class="floating-label">Lead reference</label>
      	<div>	 
 				<label id="contact9" style="display: none;"><font face="Arial" size="2" color="red">Please Enter Character Value</font></label> 
 		   </div>
...
<div class="row col-12 mt-4">
		<div class="col-3">
							<div class="input-container">
							<input id="count1" name="count1" maxlength="6"  class="form-control floating-input" type="text" placeholder="" value="<%=ooc%>" onblur="return units9();">
      <label for="count1" class="floating-label">Opportunity Count :</label>
     	<div>
				 <label id="noofunits6" style="display: none;"><font face="Arial" size="2" color="red">Please Enter Numeric Value</font></label>
				 <label id="noofunits7" style="display: none;"><font face="Arial" size="2" color="red">Please Enter Numeric Value</font></label> 
				</div>				
							</div>
					</div>
					<div class="col-3">
							...
							<input id="opportunityname" name="opportunityname" class="form-control floating-input" type="text" placeholder="" value="<%=opportunityname%>"  onblur="return contactpersonfun5();">
      <label for="count1" class="floating-label">Opportunity Name</label>
     	<div>  
      	<label id="contact5" style="display: none;"><font face="Arial" size="2" color="red">Please Enter Opportunity Name</font></label>  
 		<label id="contact6" style="display: none;"><font face="Arial" size="2" color="red">Please Enter Character Value</font></label>
</div> 	
							...
							<input class="form-control floating-input" type="text" id="email" name="email" value="<%=email%>"  placeholder="" value="<%=email%>"  onblur="return validateEmail();" required>
...
<label   id="email2" style="display: none;"><font face="Arial" size="2" color="red">Please Enter valid EmailID</font></label> 
							 ...
							 <div class="col-3 ">
        <div class="input-container">
          	 <textarea class="form-control floating-input" id="address" name="address" rows="1" placeholder=""><%=address%></textarea>
      ...
      <div class="col-3 ">
        ...
        <select class="form-select" id="country1" name="country1"  onchange="showbox();showbox2();"  required>
          	<option value="Select"><%=country1%></option>
				  ...
				  ResultSet rs7=st2.executeQuery(sqlDropdown3);
				  while(rs7.next()){
				  %>
        ...
        <select   disabled class="form-select"  id="state"  name="state"   onchange="showbox1()" > 
	           <option value="Select"><%=state%></option>
				<% String sqlDropdown2="SELECT Distinct State FROM db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"citystctryinfo order by State asc ";
				  ResultSet rs6=st2.executeQuery(sqlDropdown2);
				  while(rs6.next()){
				  %>
...
<option value="Select"><%=city%></option>
				  ...
				  ResultSet rs5=st1.executeQuery(sqlDropdown1);
				  while(rs5.next()){
				  %>
     	...
     	<div class="col-3 mt-4">
        <div class="input-container">
       		<input class="form-control floating-input" type="text" id="zip" name="zip" value="<%=zip%>"  placeholder=""  onblur="return pin();">
 ...
 <div class="row col-12 mt-4"> 
    <div class="col-3">
        <div class="input-container">
     		...
     		<input class="form-control floating-input" type="text" id="mobile" name="mobile" value="<%=mobile%>" placeholder=""  required onblur="return funmob1();">
...
<input class="form-control floating-input" type="text" id="faxno" name="faxno"  value="<%=fax%>" placeholder=""  onblur="return fax();">
        ...
        <div class="col-3">
        <div class="input-container">
        <input class="form-control floating-input" type="text"  id="std" name="std"  placeholder=""  value="<%=std%>" onblur="return units5();">
   ...
   <div class="col-3">
        <div class="input-container">
       		<input class="form-control floating-input" type="text" id="phone" name="phone"  value="<%=phone%>" placeholder=""  required onblur="return funmob2();">
          	...
          	<div>		
				...
				<label id="phone12" style="display: none;"><font face="Arial" size="2" color="red">Number Should Be 6-14 Digits</font></label>
...
String sqltemp="SELECT TheGroupName,TheGroupCode FROM db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"tbl_groupmaster where TheGroupCode='"+request.getParameter("category")+"' "; 
ResultSet rscat=st6.executeQuery(sqltemp);
if(rscat.next()){
	...
	TheGroupCodeshow=rscat.getString("TheGroupCode");
        ...
        <select id="category" name="category" class="form-select"  > 
		  	<option value="<%=TheGroupCodeshow%>"><%=TheGroupNameshow%></option>
				  ...
				  ResultSet rs4=st2.executeQuery(sqlDropdown);
				  while(rs4.next()){
				  %>
				  <option value="<%=rs4.getString(2)%>">
				  <%=rs4.getString(1)%></option>
        	...
        	<div class="col-3 mt-4">     
       <div class="input-container">
       		<input class="form-control floating-input" type="text" id="product" name="product"  value="<%=product%>" placeholder="" >
         ...
         <div class="input-container">
 		    ...
 		    <input class="form-control floating-input" type="text" id="weeklyOffOn" name="weeklyOffOn"  value="<%=weeklyoffon%>" placeholder=""  onblur="return contactpersonfun1();">
    ...
    <div class="col-3 mt-4">          
         <div class="input-container">	      
		      <input class="form-control floating-input" type="text" id="alternateNumber" name="alternateNumber" maxlength="15" value="<%=alternateNumber%>" placeholder=""  onblur="return  funmob3();">
...
<div class="col-3">       
         <div class="input-container">       
         <input id="potentialValue" name="potentialValue"  class="form-control floating-input" type="text" value="<%=potentialValue %>" placeholder="" onblur="return code1();" />
	...
	<div class="col-3">     
           ...
           <input id="noofunits" name="noofunits"  class="form-control floating-input" type="text"  placeholder="" value="<%=noofunits%>" onblur="return  units();"/>
     ...
     <div class="col-3 ">
        <div class="input-container">
          	 <textarea class="form-control floating-input" id="comments" name="comments" rows="1" placeholder=""><%=comments%></textarea>
 ...
 <div class="d-grid gap-2 col-3 mx-auto mt-3">
        <button  id="submit" name="submit" class="btn" type="submit" onclick="return chekc123()" >Submit</button>
  </div> 
</div>
	</div>
<input type="hidden" name="type111" id="type111" value="<%=Category%>">
<input type="hidden" name="company111" id="company111" value="<%=companyname%>">
<input type="hidden" name="companycode111" id="companycode111" value="<%=Custcode%>">
<input type="hidden" name="oppocode1" id="oppocode1" value="<%=request.getParameter("Opportunitycode")%>" >
<input type="hidden" name="comp" id="comp" value="<%=companyname%>">
<input type="hidden" name="cc" id="cc" value="<%=ccode%>">
<input type="hidden" name="oc" id="oc" value="<%=ooc%>">
<input type="hidden" name="custtype" id="custtype" value="<%=Category%>">
<input type="hidden" name="assignedBy" value="<%=session.getAttribute("EmpName").toString() %>"  />
<input type="hidden" name="ooporcode" value="<%=ooc%>"  />
<input type="hidden" name="companyname" id="companyname" value=<%=companyname %> >
<input type="hidden" name="customercode" id="customercode" value=<%=ccode %> >
...
function funmob1()
 ...
 function addCon(){
	 alert("1");
        ...
        var url="addcontact.jsp?companyname="+customername+"&customercode="+customercode+"&Type="+Category+"&isclose=true";
