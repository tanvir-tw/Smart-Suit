...
<%
	...
	String twEmpName=session.getAttribute("EmpName").toString();
	...
	String GetData="select * from "+session.getAttribute("CompanyMasterID").toString()+"invoicedet where InvoiceNo='"+invoiceno+"' and invoiceRefNo='"+invoiceRefno+"'";
	...
	if(rsGetData.next())
	{
		...
		String sqlDeduction="INSERT into "+session.getAttribute("CompanyMasterID").toString()+"allpaymentdetails (paymentvoucherno,paymentdate,chorddorcreditnoteno,"+
		                ...
		                "VALUES ('"+paymentVoucher+"','"+invoicedate+"','"+paymentVoucher+"','"+invoicedate+"','"+Total+"',"+invoiceno+",'"+invrefnum+"','-','-','-','Invoice Cancelled "+invrefnum+" ',"+mCustomercode+",'-','-','"+session.getAttribute("EmpName")+"')";
	   ...
	   String sqlLedger="Select balance from  "+session.getAttribute("CompanyMasterID").toString()+"customerledgeraccount  where CustomerCode='"+mCustomercode+"' ORDER BY TransactionId  desc limit 1";
		 ...
		 String sqlLedger1="INSERT INTO "+session.getAttribute("CompanyMasterID").toString()+"customerledgeraccount (CustomerCode, 	TransactionType,TransactionDetail ,TransactionDate,"+
				 ...
				 " ('"+mCustomercode+"','Invoice Reversed','"+paymentVoucher+"','"+invoicedate+"','0.00','"+Total+"','"+balance+"','"+session.getAttribute("EmpName")+"','"+paymentVoucher+"','Auto','"+mCustomercode+"')";
		 ...
		 String sqlUpdate="UPDATE "+session.getAttribute("CompanyMasterID").toString()+"invoicedet SET InvoiceCancelled=1 , ReasonForCancelling='"+cancelreason+"', ClosedBy='"+twEmpName+"'  where InvoiceNo='"+invoiceno+"' and invoiceRefNo='"+invrefnum+"'";
			
			System.out.println(sqlUpdate);
			
			st.executeUpdate(sqlUpdate);	
			
		String Updateledger="update "+session.getAttribute("CompanyMasterID").toString()+"customerledgeraccount set TransactionType='Invoice Cancelled',PaymentVoucherNo='"+paymentVoucher+"' where TransactionDetail='"+invrefnum+"' and CustomerCode='"+mCustomercode+"'";
		st5.executeUpdate(Updateledger);
		
		System.out.println(Updateledger);
	...
	/* else
	{
		...
		String sqlUpdate="UPDATE "+session.getAttribute("CompanyMasterID").toString()+"invoicedet SET InvoiceCancelled=1 , ReasonForCancelling='"+cancelreason+"', ClosedBy='"+twEmpName+"'  where InvoiceNo='"+invoiceno+"' and invoiceRefNo='"+invoiceRefno+"'";
