...
String sql4 = "select dotprojectid  from db_GlobalERP.UserMaster where dept!='null' and ActiveStatus = 'Yes' and companymasterid='"+comid+"' and dotprojectid!='-' and dotprojectid!=''";
System.out.println("sessionlist---->"+sql4);
ResultSet rs4 = st3.executeQuery(sql4);
while(rs4.next())
{
	if(c==0){
		userlist +=	"'"+rs4.getString("dotprojectid")+"'";
		
	}else{
	
	    userlist = userlist+","+"'"+rs4.getString("dotprojectid")+"'";
	}
	c++;
}
...
<div class="content-header-col-md-7-mt-10 col-12"> 
    <div class="row breadcrumbs-top">
                <h5 class="content-header-title text-center mb-0" style="color: #060637">User-wise Task Summary Report From <%=datex1%> To <%=datex2%></h5>
    </div>
</div>
 <br>
...
<%
  ...
  while(rs1.next()){
	  ...
	  String sql2="select EmpName,Dept from db_GlobalERP.UserMaster where dotprojectid='"+userId+"'";
...
<tr>
	 ...
	 <td class="text-center"><%=counter++%></td>
		...
		<td class="text-center"><a href="UserDrilldownReport.jsp?date1=<%=startDate%>&date2=<%=endDate%>&projectId=<%=projectId%>&userId=<%=userId%>&projectName=<%=projectName%>"><%=totalTaskDuration%></a></td>
					...
					buttons : [
							{
	                         ...
	                         title: 'Userwise Task Summary Report From <%=datex1%> To <%=datex2%>',
							},
							{
							 extend: 'pdfHtml5',
