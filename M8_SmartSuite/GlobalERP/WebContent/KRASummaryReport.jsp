...
// Variable Declaration
	String datex1="",datex2="", projectId="" , projectName="",startDate="",endDate=""; 
...
try{
	...
	if(submit==null){
		//onload
		datex1 = new SimpleDateFormat("01-MMM-yyyy").format(new java.util.Date());
	...
	startDate = new SimpleDateFormat("yyyy-MM-dd").format(new SimpleDateFormat("dd-MMM-yyyy").parse(datex1));
	endDate = new SimpleDateFormat("yyyy-MM-dd").format(new SimpleDateFormat("dd-MMM-yyyy").parse(datex2));
	
	String userlist = "(0,";
	int c=0;

	String sql3 = "select KRAdotprojectid  from db_GlobalERP.UserMaster where dept!='null' and ActiveStatus = 'Yes' and companymasterid='"+session.getAttribute("CompanyMasterID").toString()+"' and KRAdotprojectid!='-' and KRAdotprojectid!=''";
	System.out.println("sessionlist---->"+sql3);
	ResultSet rs3 = st3.executeQuery(sql3);
	while(rs3.next())
	{
		if(c==0){
			userlist +=	"'"+rs3.getString("KRAdotprojectid")+"'";
			
		}else{
		
		    userlist = userlist+","+"'"+rs3.getString("KRAdotprojectid")+"'";
		}
		c++;
	}
...
<div class="content-header-col-md-7-mt-10 col-12"> 
    <div class="row breadcrumbs-top">
                <h5 class="content-header-title text-center mb-0" style="color: #060637">Project-wise KRA Summary Report</h5>
    </div>
</div>
...
<table id="example" class="display" style="width:100%;">        
      ...
      <%
  int counter=1;
  String sql1= "select task_project,project_name,sum(task_duration) as duration from dotproject.tasks a  join dotproject.user_tasks b on a.task_id = b.task_id join dotproject.projects c  on a.task_project=c.project_id where  task_end_date >='"+startDate+" 00:00:00'  and task_end_date <='"+endDate+" 23:59:59' and b.user_id in "+userlist+"   group by task_project order by task_project";
...
<tr>
		...
		<td class="text-left"><a href="UserwiseKRAReport.jsp?date1=<%=startDate%>&date2=<%=endDate%>&projectId=<%=projectId%>"><%=projectName%></a></td>
					...
					buttons : [
							{
	                         ...
	                         className: 'dropdown-item',  
		                        title: 'Projectwise KRA Summary Report '+$("#data").val()+' to ' +$("#data1").val(),		                        
							},
							{
							 extend: 'pdfHtml5',
