...
String fromDate="",sqlEnq="";
String SalesEnqRefNo="",SalesEnqDate="",SalesEnqType="",CompanyName="",CustRefNo="",TWEmpName="",SalesEnqNo="";
int i=1;
...
<div class="content-header-col-md-7-mt-10 col-12">
    <div class="row breadcrumbs-top">
                <h5 class="content-header-title text-center mb-0" style="color: #060637">Generate Sales Quotation From Sales Enquiries</h5>
    </div>
</div>
  <form name="data" id="formreport" method="post">
...
<table id="example" class="display" width="100%">        
       <thead>
		...
		sqlEnq="SELECT a.SalesEnqRefNo, a.SalesEnqDate, a.SalesEnqType, a.CompanyName, a.CustRefNo, a.TWEmpName ,a.SalesEnqNo FROM db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"salesenq_det AS a  LEFT OUTER JOIN db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"salesenq_items AS b ON ( a.SalesEnqNo = b.SalesEnqNo ) WHERE a.CompanyName like '%%' and a.SalesEnqDate BETWEEN '"+dt+"' AND '"+dt1+"'  GROUP BY a.SalesEnqRefNo, a.SalesEnqDate, a.SalesEnqType, a.CompanyName, a.CustRefNo, a.TWEmpName ,a.SalesEnqNo ";
    	System.out.println(sqlEnq);			
    	ResultSet rsEnq=st1.executeQuery(sqlEnq);
   
    	while(rsEnq.next()){ 
    	
    		SalesEnqRefNo=rsEnq.getString("SalesEnqRefNo");
    		SalesEnqDate=rsEnq.getString("SalesEnqDate");
    		 if(SalesEnqDate =="-" || SalesEnqDate.equalsIgnoreCase("-") || SalesEnqDate==null){
    			 SalesEnqDate ="-";
	    		 
	    	 }else{
	    		 SalesEnqDate=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(SalesEnqDate));
	    	 }
    		SalesEnqType=rsEnq.getString("SalesEnqType");
    		CompanyName=rsEnq.getString("CompanyName");
    		CustRefNo=rsEnq.getString("CustRefNo");
    		TWEmpName=rsEnq.getString("TWEmpName");
    		SalesEnqNo=rsEnq.getString("SalesEnqNo");
    	%>
         <tr> 
         		<td class="text-center"><%=i%></td>
				<td class="text-left"><a href="enqdet.jsp?companyName=<%=CompanyName%>&SalesEnqNo=<%=SalesEnqNo%>&SalesEnqRefNo=<%=SalesEnqRefNo%>" target="_blank"> <%=SalesEnqRefNo%></a></td>
                <td class="text-right"><%=SalesEnqDate%></td>
                <td class="text-center"><%=SalesEnqType%></td>
					...
					buttons : [
							{
	                         ...
	                         className: 'dropdown-item',  
								title: 'Sales Quotation From Sales Enquiries Report from '+$("#data").val()+ ' to ' +$("#data1").val(),
							},
							{
							 extend: 'pdfHtml5',
