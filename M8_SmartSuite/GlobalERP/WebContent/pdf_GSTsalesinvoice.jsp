...
<%
    ...
    Statement st = null, st1 = null,st2=null,st3=null,st4=null,st5=null,st6=null,st7=null,st8=null,stlbl=null;
System.out.println("HHHHHHHHHHHHHHHHHEEEEEEEEEEE");
...
System.out.println("GST  Ivoice PDF:- ");

String compmaster="select *  from CompanyMaster where companymid='"+session.getAttribute("CompanyMasterID").toString()+"'";
ResultSet rscompmaster=st5.executeQuery(compmaster);

System.out.println("CompanyMaster :- "+compmaster);
...
String SqlQuoDet="SELECT InvoiceNo,SalesOrderNo,CustomerCode,InvoiceDate,TheGroup,Total,Discount,CST,PFI,FreightAmt,GrandTotal,Terms,TotalInWords,PONo,PODate,DespatchMode,FreightType,PaymentMode,Insurance,ShippedBy,ShipmentDate,DespCheckListCheckedBy,PackingSlipCheckedBy,DocketFilledBy,InvoiceCancelled,RevofInvoiceNo,NoofRevisions,ReasonForCancelling,SEandSQRefNo,InvoiceTime,InvoiceRaisedby,Annexure,ExpIECno,RBIcodeNo,OtherExpInfo,BuyerOtherThanConsig,CountryOfOrigin,CountryOfFinalDestin,PreCarriageBy,PlaceofRecptByPreCar,PortOfLoading,PortOfDischarge,FinalDestination,NoandKindOfPacking,WeightInKgs,CurrType,CurrValue,TWSalesOrderNo,DebitNote,TWEmpName,AddressCode,CompanyName,ContactPerson,Address,city,Zip,State,country,Phone,Fax,invoiceRefNo,TWSalesorderRefNo,RevOfinvoiceRefNo,SalesInTransit,VATdeclaration,UpdatedinTally,InvoiceType,InvoicePrinted,TallyRefNo,Newcustomercode,Newcompanyname,CustPurNO,CustPurDT,InvRaisedBillAdd,InvRaisedShipAdd,InvRaisedConDet,InvRaisedCompName,ClosedBy,HSNCode,SACCode,CGSTRate,CGSTValue,SGSTRate,SGSTValue,IGSTRate,IGSTValue,BillingGSTIN,ShippingGSTIN,CASE WHEN FromDate ='0000-00-00' THEN null ELSE FromDate END as FromDate,CASE WHEN ToDate ='0000-00-00' THEN null ELSE ToDate END as ToDate,CASE WHEN DeliveryDate ='0000-00-00' THEN null ELSE DeliveryDate END as DeliveryDate,CASE WHEN DueDate ='0000-00-00' THEN null ELSE DueDate END as DueDate from "+session.getAttribute("CompanyMasterID").toString()+"invoicedet WHERE invoiceRefNo='"+invoicerefno+"' ";
...
ResultSet rsInvDet=st4.executeQuery(SqlQuoDet);
	if(rsInvDet.next()){
		...
		String f11=rsInvDet.getString("CustPurNO");
		if(f11==null || f11=="null" || f11.equals("null"))
		{
			System.out.println("f11 null  =======>" + f11);
			f11="0";
		...
		String todaysDate1=new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new java.util.Date());
		String paylink="",expirydate1="";
		
		 String query2="select * from db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"PayLinkDetails where invoice_id='"+invoicerefno+"' and expire_by >= '"+todaysDate1+"'";
	...
	{
		paylink="Link Is Expired Create Link";
		System.out.println(""+paylink);
		String query3="select * from db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"PayLinkDetails where invoice_id='"+invoicerefno+"' order by SrNo desc limit 1";
	
		
		  System.out.println("query3-----"+query3);
ResultSet rsquery3=st8.executeQuery(query3);
		if(rsquery3.next())
		{
		expirydate1=rsquery3.getString("expire_by");
		}
		else{
			paylink="-";
			expirydate1="-";
...
document.add(NF2);	
	...
	firstcell1.addElement(new Paragraph("PO No     :     "+f11,Fontforname));
	firstcell1.setHorizontalAlignment(Element.ALIGN_RIGHT);
	...
	double TotAmount=0.00;
		while(rsInvItems.next())
		{
			...
			System.out.println("INsqlGroup IF   "+sqlGroup);
			...
			if(groupCode<5)
			{
				System.out.println("IN IF   "+sqlDisplayDesc);
				...
				System.out.println("sqlMakename IF   "+sqlMakename);
				...
				System.out.println("IN  else   "+sqlDisplayDesc);
		...
		System.out.println(b);
		   ...
		   h=rsInvItems.getString("SellingPrice") ;
			...
			System.out.println(m);
				      ...
				      double amt=(sp*qty);
				      double discamt=((dp/100)*amt);
				      double taxableamount=(sp*qty)-((dp/100)*amt);
				      ...
				      double pf=Double.parseDouble(PFI);
				       d1=(totamt*pf)/100;
				      System.out.println("d1->"+d1);				      double disval=(sp-((dp/100)*amt));
...
Paragraph parabottom12=new Paragraph();
parabottom12.add(new Paragraph(" ",Normal));
parabottom12.setAlignment("Right");
document.add(parabottom12);


try
{
Image image1=null;
PreparedStatement ps = conn.prepareStatement("Select Signature from db_GlobalERP.UserMaster where EMPName='"+rsInvDet.getString("TWEmpName")+"' and companymasterid='"+session.getAttribute("CompanyMasterID").toString()+"' ");
...
if(rs.next())
{
	Blob b21=rs.getBlob("Signature");//2 means 2nd column data  
    byte barr[]=b21.getBytes(1,(int)b21.length());//1 means first image  
...
String paynotes1="1.Using Payment Link You Can Pay Via Card/EMI."
+"\n\t"+"2.Using Payment Link You Can Pay Via Netbanking."
+"\n\t"+"3.Using Payment Link You Can Pay Via Wallet."
+"\n\t"+"4.Using Payment Link You Can Pay Via UPI.\n";
...
Paragraph linkdetails=new Paragraph();
linkdetails.add(new Paragraph("PAYMENT LINK DETAILS :",NormalFont1));
linkdetails.setAlignment("Left");
document.add(linkdetails);

Paragraph paralink=new Paragraph();
paralink.add(new Paragraph("Link:  ",NormalFont1));
paralink.add(new Paragraph(""+ paylink +"",NormalFont21));
paralink.setAlignment("Left");
document.add(paralink);

Paragraph paralinkexpiry=new Paragraph();
paralinkexpiry.add(new Paragraph("Link Expiry Date:  ",NormalFont1));
paralinkexpiry.add(new Paragraph(""+ expirydate1 +"",NormalFont21));
paralinkexpiry.setAlignment("Left");
document.add(paralinkexpiry);



Paragraph paynotes=new Paragraph();
paynotes.add(new Paragraph("Pay Via Link:",NormalFont1));
paynotes.add(new Paragraph(""+ paynotes1 +"",NormalFont21));
paynotes.setAlignment("Left");
document.add(paynotes);
...
if(InvoiceType.equals("ASCInvoice") || InvoiceType.equals("ASCShortSummary"))
{
	...
	//c1.setFixedWidth(); 
	table1.addCell(c2);
	c2 = new PdfPCell(new Phrase(rslbl1.getString("col11"),NormalFont1));
	c2.setHorizontalAlignment(Element.ALIGN_CENTER);
	//c1.setFixedWidth(); 
	table1.addCell(c2);
	...
	float[] widthss1 = {0.02f, 0.05f, 0.05f, 0.05f,0.04f,0.05f,0.06f};
	...
	paratotal3.add(new Paragraph(" Total (Amount Before Tax): "+TotalAmount+" ",Normal));
