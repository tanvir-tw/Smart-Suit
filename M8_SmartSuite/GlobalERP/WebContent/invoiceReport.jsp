...
String sqlinvoice="",sign="",SalesOrderTime="",SalesOrderNo="",Total="",currType="",TheGroup="",companyName="",SalesOrderDate="",SalesorderRefNo="";
int i=1; 
...
<div class="content-header-col-md-7-mt-10 col-12">
    <div class="row breadcrumbs-top">
    	<%if(condition.equalsIgnoreCase("proforma")) { %>
          <h5 class="content-header-title text-center mb-0" style="color: #060637">Generate Proforma Invoice from Sales Order</h5>
         <%} else if(condition.equalsIgnoreCase("main")) {%>
           <h5 class="content-header-title text-center mb-0" style="color: #060637">Generate Invoice form Sales Order</h5>
         <%} %> 
    </div>
</div>
...
<table id="example" class="display" style="width:100%;">        
       <thead>
        ...
        <tbody>
        <%
    	sqlinvoice="SELECT * FROM "+session.getAttribute("CompanyMasterID").toString()+"salesorder_det as a "+
    			" LEFT OUTER JOIN "+session.getAttribute("CompanyMasterID").toString()+"salesorder_items as b ON(a.SalesOrderNo=b.SalesOrderNo) "+
    			" WHERE a.SOCancelled=0 AND a.InvoiceGenerated=0 AND a.SalesOrderDate BETWEEN '"+dt+"' and '"+dt1+"' "+
    	...
    	ResultSet rs=st1.executeQuery(sqlinvoice);
   
    	while(rs.next()){ 
    		sign=rs.getString("TWEmpName");
			if(sign==null || sign.equalsIgnoreCase("null"))
			{
				sign="-";
			}
			Total=rs.getString("Total");
			currType=rs.getString("currType");
			TheGroup=rs.getString("TheGroup");
			...
			SalesOrderTime=rs.getString("SalesOrderTime");
         ...
         <tr> 
         	    <td class="text-center"><%=i%></td>
			       ...
			       if(rs.getString("SalesorderRefNo").contains("EXP")){%>
							<td class="text-left"><a href="Expinvoicedet.jsp?salesOrderNumber=<%=SalesOrderNo%>&condition=<%=condition%>&currT=<%=currType%>" target="_blank"> <%=SalesorderRefNo%></a></td>
				    <%}else{%>
					        <td class="text-left"><a href="invoicedet.jsp?salesOrderNumber=<%=SalesOrderNo%>&condition=<%=condition%>" target="_blank"> <%=SalesorderRefNo%></a></td>
				    <%}%>
         	    <td class="text-right"><%=SalesOrderDate%> <%=SalesOrderTime%></td>
					...
					buttons : [
							{
	                         ...
	                         className: 'dropdown-item',  
								title: 'Sales Order Report from '+$("#data").val()+ ' to ' +$("#data1").val(),
							},
							{
							 extend: 'pdfHtml5',
