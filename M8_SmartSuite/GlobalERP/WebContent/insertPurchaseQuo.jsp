...
try{	
	...
	String discount1="",tax1="0",amount="",total="",salesquono="";
...
//  ++++++++++++++++++++++++++++++++++++++++++++++   TW REF NO NOT ADDED YET   ********************************************************************
	...
	String sqlInsert="INSERT INTO "+session.getAttribute("CompanyMasterID").toString()+"purchasequo (PurQuoNo ,TWRefNo ,SupplierCode ,PurQuoSupRefNo ,PurQuoDate ,Prompt ,TaxRate, "+
					 ...
					 "TotCGST,TotSGST,TotIGST,TotGST )"+
			
					 "VALUES ('"+pquono+"','"+twrefno+"','"+supcode+"','-','"+purchasedate+"','0','0.00',"+
							 "'"+pfi+"','-','-','0.00','"+paymentterms+"','"+terms+"','0.00','0.00',"+
					 "'-','"+frieght+"','-','0','0','0','"+grandtotal+"','Rupees','1','"+totcgstamt+"','"+totsgstamt+"','"+totigstamt+"','"+totgstamt+"' )";			
	System.out.println("Quo Master :- "+sqlInsert);	
	...
	for(i=0;i<number;i++)
	{
		...
		cgstrate=request.getParameter("CGST"+i);
		sgstrate=request.getParameter("SGST"+i);
		igstrate=request.getParameter("IGST"+i);
		...
		String sql="INSERT INTO "+session.getAttribute("CompanyMasterID").toString()+"pquo_items (PQNo ,TheGroup ,Make ,ItemCode ,Quantity ,UnitPrice ,DiscountPercent ,"+
				   "TheTableToBeAccessed ,STPercent ,LandedPrice ,ExtraTaxesApplicable,"+
					"CGSTRate,SGSTRate,IGSTRate,CGSTAmt,SGSTAmt,IGSTAmt,ServiceCategary,SACCode,HSNCode)"+
				   
				   "VALUES ('"+pquono+"','"+no+"','"+mcode+"','"+itemcode+"','"+quantity1+"','"+sellingprice1+"','"+discount1+"',"+
				   " '"+tableToBeAccessed+"','"+tax1+"','"+sellingprice1+"','0',"+cgstrate+","+sgstrate+","+igstrate+","+cgstamt+","+sgstamt+","+igstamt+",'"+ServiceCat+"','"+SACCode+"','"+HSNCode+"' )";
		System.out.println("Quo Item :-" +sql);
